...
**Orient first.** Read the repo `_Mode Mode/modemode` (request the `modemode` folder if you can't read it): `docs/STATUS.md` (live snapshot), `docs/ROADMAP.md`, `docs/ORCHESTRATION.md`, `docs/project-pages_architecture_2026-06-26.md` (incl. §8), `docs/cms-plan.md`, the `docs/handoff_*.md`, and your saved memory. Then `index.html` / `project.html` as needed. Confirm current state before acting. Live at GitHub Pages off `git@github.com:jalulia/modemode.git`.

**The system — don't break it.** One self-contained JSON per project (`content/<slug>.json`); all data through a single `loadProject()` boundary; media referenced by URL (keep it Supabase-portable). The homepage metaball is the source of truth for the project-page minimap — mirror it read-only, never refactor `index.html`'s renderer. BB Strata type, white background, the iOS light workaround, the hairline / JetBrains-Mono brand system. The MODE MODE logo is one fixed lockup that links home.
