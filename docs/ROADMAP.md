...
5. **Dark mode on `project.html`** (6 steps, in order): ① unify the token set with `index.html` + make `legibleInk()` ground-aware ✅ · ② teach the solver to *lighten* for dark grounds · ③ `body.darkbg` token block · ④ remaining hardcoded whites (minimap label chips, `.car-btn`, editor chrome) · ⑤ the blob edge — `adaptEdge()` in the renderer, the only non-CSS item, and it touches the READ-ONLY minimap mirror so scope it alone · ⑥ ship behind `?dark`. **Two decisions before ⑥ goes public:** `color-scheme:light` is a frozen invariant (the iOS light-render workaround) and would need to become `light dark` plus an iOS re-test; and *per-project* ground — the better end state, since P-01/P-04 read dark and P-05/P-06 don't — is a **schema change** (new field + editor control + 6 docs + 6 Supabase rows), so it lands after ⑥, not inside it.
