...
- **Rail top-left alignment (per Julia's mockup):** the back control is now a clean **← arrow** (was diamond + text), **outdented ~34px into the left gutter** toward the corner mark; the logo, P-04, and title hold one shared content edge. Rail `overflow:visible` (flex-fit still keeps it no-scroll, verified 0px). Applies to all project pages.
