...
- **Schema:** one `public.projects` table (`slug` pk · `code` · `name` · `data jsonb` · `updated_at` + touch trigger). RLS = public read / authenticated write. Storage bucket `project-media` (public) ready for the media migration.
- **Seeded:** all 6 projects upserted from `content/*.json`; node counts verified **15/10/2/5/5/5** (content-bearing 10/4/1/1/2/4).
- **Wiring:** `loadProject()` reads Supabase via PostgREST anon and **falls back to the bundled `content/<slug>.json`** if Supabase is ever unreachable — the site can't break, and the JSONs stay as backup + git history. Anon key is public-safe (RLS protects writes).
- **Verified (chromium):** selected-arcade renders from Supabase, anon REST read = 200 + correct data, **0 console errors**.
- **Authoring now:** Supabase → **Table editor → `projects` → `data`** (the JSON cell) — the zero-build "table UI" path. The on-brand studio block-editor is the later build.
- **On Julia:** (1) commit `project.html` + `docs/supabase/`; (2) free-tier project **idle-pauses after 7 days** — upgrade the org to **Pro ($25/mo)** in the dashboard for always-on; (3) advisor WARNs are by-design (auth-write policy; public-bucket listing — revisit listing in the media phase).

## Project page — ambient field rebuild (2026-06-28 evening, pre-CMS feedback round)
Rebuilt the left-rail constellation per Julia's mock #2. The blob is now a **large, borderless ambient field** that fills the rail and **bleeds behind the logo + title** (was a small boxed thumbnail with an `OBJECT FIELD` caption — both removed). It **animates** (undulation + asymmetric breathing) and carries a **perlin cloud texture**, ported READ-ONLY from `index.html`'s renderer (`nodeSeeds`/`nodePert` + fBm value-noise) into `project.html` — `index.html` untouched. Adaptive 3:2 flex split keeps the field large while the nav still fits any node count. **Verified headless (chromium):** selected-arcade / eyeknow (15 nodes) / gif (2 nodes) → **0 console errors, rail overflow 0px** on all, animation confirmed (frame deltas), low-res field recompute throttled to 28fps so JS self-time stays negligible. Touches `project.html` (left-rail CSS + minimap engine only) — disjoint from the in-flight lightbox (main-content region). Committed. **Feedback round 2 (2026-06-28 late):** (1) axis hatch-marks now **stop at the labels** — knockout in the field's own bg colour + lifted above the hatch (`index.html` `.axis`), sits on top of the other instance's hatch-to-edge change. (2) The project field now **bleeds off the left screen edge and floats behind the content** as a fixed backdrop sized to the content column's left edge (+46px peek), so the blob curve reads through the gaps between the opaque cards but not behind them. Both verified headless (chromium): 0 console errors, 0px rail overflow, field left=0 / width→content edge. **Remaining homepage item:** cursor becomes the "view project" tab on blob hover (no hand) — still queued.
