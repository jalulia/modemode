...
## Last touched
- **2026-07-19** — Canvas crop button (`5b53ef9`). Crop was only in the Form editor, so clicking an image in Canvas only offered the URL-swap prompt — undiscoverable. Now each image block's Canvas hover-toolbar has a **Crop** button (◱) that opens an inline crop panel (aspect + zoom + drag focal point), writing `crop` into the doc via `postDoc()`. Syntax-verified on-disk (compile-clean); it's a port of the Form crop control (already headless-verified), but the click-through interaction is not yet headless-tested — confirm live.
