...
- **Rail top-left alignment (per Julia's mockup):** the back control is a clean **← arrow** (was diamond + text), **outdented ~34px into the left gutter** toward the corner mark and **vertically on the P-04 line** (centers match exactly); the logo, P-04, and title hold one shared content edge. Rail `overflow:visible` (flex-fit still keeps it no-scroll, verified 0px). Applies to all project pages.
