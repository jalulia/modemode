...
- **One QA finding (needs a fix):** the global **hero-as-header crops NIGHTMARE KART's portrait elevation-drawing** hero into an awkward banner. Photo heroes are great; the fix is to banner only landscape rasters and `contain`/card-fallback SVG/portrait heroes (or give NK a landscape photo hero). **Julia's call on treatment.**
